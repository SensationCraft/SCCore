...
public class XraySpyCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command cmd, String command, String[] args) {
        ...
        if (xrayManager.getXraySpyers().contains(player.getUniqueId())) {
            xrayManager.getXraySpyers().remove(player.getUniqueId());
            sender.sendMessage("§cYou have disabled xray spy.");
            return false;
        } else {
            xrayManager.getXraySpyers().add(player.getUniqueId());
