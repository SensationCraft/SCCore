                ...
                if (punishment.getType().equals(PunishmentType.BAN)
                    ...
                    if (!punishment.hasExpired()) {
                        ...
                        if (hover) {
                            ...
                            message = message.then(senderSCPlayer.getTag()).tooltip(senderSCPlayer.getHoverText()).then(" has unbanned ").color(ChatColor.GRAY)
                                    .then(scPlayer.getTag()).tooltip(scPlayer.getHoverText()).then(".").color(ChatColor.GRAY);
                        } else {
                            message = message.then("Console").color(ChatColor.GOLD).then(" has unbanned ").color(ChatColor.GRAY).then(scPlayer.getTag())
