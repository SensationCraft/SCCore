...
public class DuelListeners implements Listener {
	...
	public void onPlayerQuit(final PlayerQuitEvent e) {
		...
		if (this.arena.isRunning() && this.arena.getArenaPlayers().contains(e.getPlayer())) {
			SCPlayer p = this.scPlayerManager.getSCPlayer(e.getPlayer().getUniqueId());
			e.getPlayer().setHealth(0);
			this.scPlayerManager.broadcast(new FancyMessage(p.getTag()).tooltip(p.getHoverText()).color(ChatColor.DARK_PURPLE).then(" §d5has logged while in the duel arena!"));
			this.arena.forceEnd();
		}
	...
	public void onPlayerDeath(final PlayerDeathEvent e) {
		if (this.arena.isRunning() && this.arena.getArenaPlayers().contains(e.getEntity())) {
