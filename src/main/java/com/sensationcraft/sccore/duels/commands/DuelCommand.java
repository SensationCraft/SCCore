        ...
        if (args[0].equalsIgnoreCase("request") || args[0].equalsIgnoreCase("accept") || args[0].equalsIgnoreCase
            ...
            if (args[0].equalsIgnoreCase("request")) {
                ...
                if (!target.getGameMode().equals(GameMode.SURVIVAL)) {
                    FancyMessage message = new FancyMessage(scTarget.getTag()).tooltip(scTarget.getHoverText())
                            .then(" is currently in a gamemode which prohibits you from dueling each other.").color(ChatColor.RED);
                    message.send(sender);
                    return false;
                }

                if (!player.getGameMode().equals(GameMode.SURVIVAL)) {
                    sender.sendMessage("§cYou are only allowed to duel while in the survival gamemode.");
                    return false;
                }
