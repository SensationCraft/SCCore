...
ConfigurationSerializable {
	...
	public FancyMessage(final TextualComponent firstPartText) {
		...
		if (FancyMessage.nmsPacketPlayOutChatConstructor == null) {
			try {
				FancyMessage.nmsPacketPlayOutChatConstructor = Reflection.getNMSClass("PacketPlayOutChat").getDeclaredConstructor(Reflection.getNMSClass("IChatBaseComponent"));
				FancyMessage.nmsPacketPlayOutChatConstructor.setAccessible(true);
	...
	public static FancyMessage deserialize(String json) {
		JsonObject serialized = FancyMessage._stringParser.parse(json).getAsJsonObject();
			...
			for (Map.Entry<String, JsonElement> entry : messagePart.entrySet()) {
				...
				if (TextualComponent.isTextKey(entry.getKey())) {
					...
					if (object.get("value").isJsonPrimitive()) {
						...
						component.hoverActionData = FancyMessage.deserialize(object.get("value").toString() /* This should properly serialize the JSON object as a JSON string */);
					}
					...
					for (JsonElement object : entry.getValue().getAsJsonArray()) {
						if (object.isJsonPrimitive()) {
							...
							component.translationReplacements.add(FancyMessage.deserialize(object.toString()));
						}
					}
				}
			}
	...
	private Object createChatPacket(String json) throws IllegalArgumentException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException, ClassNotFoundException {
		...
		Object serializedChatComponent = FancyMessage.fromJsonMethod.invoke(FancyMessage.nmsChatSerializerGsonInstance, json, Reflection.getNMSClass("IChatBaseComponent"));
