    ...
    public void onLateChat(final AsyncPlayerChatEvent event) {
        ...
        switch (user.getChannel()) {
            ...
            case ALLY:
                ...
                FancyMessage message2 = new FancyMessage("§5[").then(mPlayer.getFactionName()).then("§5] " + FactionUtil.getAsteriskPrefix(mPlayer)).then(user.getTag()).tooltip(user.getHoverText())
                        .then(": " + event.getMessage(), true);
